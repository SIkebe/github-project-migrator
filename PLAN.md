...
| Item(Issue / PR / Draft) | GraphQL | Issue/PR はリポジトリマッピング CSV で解決。Draft は本文冒頭に元作成者・日時を注記 |
...
| コラボレーター / リポジトリリンク | GraphQL + 将来 UI | import は `updateProjectV2Collaborators` / `linkProjectV2ToRepository`(ユーザー/リポジトリマッピング CSV 適用、解決不能は warning+skip)。**コラボレーターの読み取り API は存在しない**(`ProjectV2ActorConnection` は mutation payload のみ、実スキーマで確認 2026-07-06)ため API export は常に null — v1 は手書きスナップショットでのみ import 可能。ただし `/settings/access` UI には明示 collaborator と role が表示されることを `ravel-maurice-uo_sde` で検証済み(2026-07-06)のため、v1.x で Playwright export 候補。Base role / org owner / inherited access は対象外。チームリンク(`linkProjectV2ToTeam`)はコラボレーターとは別概念で v1 対象外 |
...
| Issue/PR 本体の移行 | 本ツールのスコープ外(GitHub Enterprise Importer の役割)。リポジトリマッピング CSV で接続 |
