...
public class CsvMappingTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "ghpmv-mapping-" + Guid.NewGuid().ToString("N") + ".csv");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "ghpmv-user-mapping-" + Guid.NewGuid().ToString("N") + ".csv");
