...
|---|---|---|
| `ProjectV2FieldType` does not contain `MULTI_SELECT`. | Selecting `ProjectV2Field.dataType` for a linked multi-select Issue Field returns a generic GraphQL internal error instead of a usable type. Some projects also fail when reading the complete field connection after the link exists. Native Project multi-select fields cannot yet be represented reliably by the API. | Export and import first read field identity without `dataType`, then resolve potentially linked candidates individually. Import falls back to per-name field lookup when the connection returns the known preview error. Only the node that returns the type error remains untyped, preserving unrelated normal Project fields. |
