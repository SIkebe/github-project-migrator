...
- `ghpmv login --browser`: headful Chromium で `{base}/login` を開き、ユーザーが手動ログイン(2FA/SSO/passkey 込み)。ログイン完了は `{base}` に遷移後 `document.querySelector('meta[name="user-login"]')` が非空になることで検知 → `IBrowserContext.StorageStateAsync()` を `~/.ghpmv/storage-state.json` に保存(ファイル権限 600)。
- 以降の実行は `Browser.NewContextAsync(new() { StorageStatePath = ... })`。コンテキスト生成直後にプロジェクト URL を開き、ログイン画面へリダイレクトされたら「セッション失効。`ghpmv login --browser` を再実行してください」と即座に失敗させる。
- SAML SSO 組織では storageState に SSO セッションが含まれるため、ログイン時に**対象 org のプロジェクト URL を開かせて SSO 認可まで完了させる**こと。
- GHEC with data residency: セッションはホスト単位のため storageState を `~/.ghpmv/storage-state.{host}.json` とホスト別に保存し、`ghpmv login --browser --base-url https://{tenant}.ghe.com` でテナント側のログイン(EMU/IdP 経由)も別途実施する。GitHub.com(export 元)と DR テナント(import 先)の 2 セッションを使い分ける。
...
`ghpmv export` は `--skip-browser` 指定時に `ui` セクションを null にして API のみで完了できる(その場合 import 側でも該当項目をスキップし警告)。
...
| B2 | `ghpmv login --browser` / `ghpmv setup --browsers` | 手動確認 + セッション失効検知の E2E |
