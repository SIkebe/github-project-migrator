...
- `gpm login --browser`: headful Chromium で `{base}/login` を開き、ユーザーが手動ログイン(2FA/SSO/passkey 込み)。ログイン完了は `{base}` に遷移後 `document.querySelector('meta[name="user-login"]')` が非空になることで検知 → `IBrowserContext.StorageStateAsync()` を `~/.gpm/storage-state.json` に保存(ファイル権限 600)。
- 以降の実行は `Browser.NewContextAsync(new() { StorageStatePath = ... })`。コンテキスト生成直後にプロジェクト URL を開き、ログイン画面へリダイレクトされたら「セッション失効。`gpm login --browser` を再実行してください」と即座に失敗させる。
- SAML SSO 組織では storageState に SSO セッションが含まれるため、ログイン時に**対象 org のプロジェクト URL を開かせて SSO 認可まで完了させる**こと。
- GHEC with data residency: セッションはホスト単位のため storageState を `~/.gpm/storage-state.{host}.json` とホスト別に保存し、`gpm login --browser --base-url https://{tenant}.ghe.com` でテナント側のログイン(EMU/IdP 経由)も別途実施する。GitHub.com(export 元)と DR テナント(import 先)の 2 セッションを使い分ける。
...
- **失敗時診断(全操作共通)**: 例外捕捉時に ①`page.Url` ②`page.ScreenshotAsync()` ③`page.Locator("body").AriaSnapshotAsync()` ④console ログ、を `./gpm-diagnostics/{timestamp}/` に保存してから再スロー。リトライは**ページリロード → 操作全体をやり直し**を 1 回まで(要素単位の盲目リトライはしない)。
...
1. フィクスチャープロジェクト(§7)を `GPM_TEST_ORG` に手動 or スクリプトで作成
2. `pwsh playwright.ps1 codegen --load-storage=storage-state.json https://github.com/orgs/$env:GPM_TEST_ORG/projects/1` で以下の操作を一通り録画し、生成されたセレクターを控える:
...
`gpm export` は `--skip-browser` 指定時に `ui` セクションを null にして API のみで完了できる(その場合 import 側でも該当項目をスキップし警告)。
...
`GPM_TEST_ORG` に作る基準プロジェクト(セットアップスクリプトは可能な限り GraphQL、View/Workflow 部分は初回手動 + 本ツール自身でのブートストラップ):
...
| B2 | `gpm login --browser` / `gpm setup --browsers` | 手動確認 + セッション失効検知の E2E |
